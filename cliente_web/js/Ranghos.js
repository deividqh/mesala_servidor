 ...
 * @example  const celdas = new Working_Celdas(api_rangos);
 */
class Working_Celdas {
	...
	constructor(api_rangos = null) {
		if (typeof api_rangos?.get_dimension !== 'function') {
			throw new TypeError('Rangos necesita la función get_dimension().');
		}
		if (typeof api_rangos?.get_matriz !== 'function') {
			throw new TypeError('Rangos necesita la función get_matriz().');
		}

		this.api_rangos = api_rangos;
		this.celda_mapper = new Celda_Mapper();
	}

	/** ### Funcion que se comunica con Salon para obtener la dimension */
	_get_dimension_matriz() {
		return this.api_rangos.get_dimension();
	}
	
	/** ### Funcion que se comunica con Salon para obtener la "matriz_plana" */
	_get_matriz_plana() {
		return this.api_rangos.get_matriz();
	}
	...
	_get_indice(fila, columna) {
		const dimension_matriz = this._get_dimension_matriz();
		const matriz_plana = this._get_matriz_plana();

		const indice = this.celda_mapper.coordenadas_a_indice(fila, columna, dimension_matriz);
		if (Array.isArray(matriz_plana) && indice !== null && indice < matriz_plana.length) {
			return indice;
		}
		return false;
	}
	...
	*/
	 ...
	 * ```javascript
	 *	matriz.X_to_indice_matriz(5); ► (Indice) Devuelve 5 si es válido
	 *	matriz.X_to_indice_matriz(2, 3); ► (row-col) Devuelve el índice de fila 2, columna 3
	 *	matriz.X_to_indice_matriz("B3"); ► (str) Devuelve el índice de la celda B3
	 *	matriz.X_to_indice_matriz({1, 4}); ► Devuelve el índice de la celda (1,4)
	 * ``` 
	*/
    X_to_indice_matriz(arg1, arg2 = null) {        
		let fila, col;
		try {
	...
	X_to_fc(arg1, arg2=null){
		// Valida
		const indice = this.X_to_indice_matriz(arg1, arg2);
		if (indice===false) return null;
		// Calcula coordenadas
		const dim = this._get_dimension_matriz?.();
	...
	*/
	_normalizar_dimension(arg1 = null, arg2 = null) {
		...
		// Ahora Valido los límites de la matriz(con la api)
		const limites = this._get_dimension_matriz();
	...
	*/
	...
	plus(arg1='A0', plus = 0) {
		...
		const indice = this.X_to_indice_matriz(arg1);
...
■■■■■■■■ suma_fc (suma una fila o columna a partir de una celda)
 const celda_suma = RAN.suma_fc('C3', 0, 1); 
 const celda_suma_mat = RAN.suma_fc({fila:3, columna:2}, 0, 1); 
...
■■■■■■■■ PLUS() (suma un indice a una celda), devuelve celda, siempre da una celda ok dentro del rango excepto en los extremos.
 const celda1 = 'C3'; 
 const plus = 7;
 const celda_plus = RAN.plus(celda1, plus); 
 if(celda_plus) 
 	console.log(`▶️ PLUS celda: ${celda1} - plus: ${plus} ► ${celda_plus}`);
■■■■■■■■ RESERVAS-A-RANGOS()
 const rangos_reserva = RAN._reservas_a_rangos(dicc_api_foto.reservas);
...
■■■■■■■■ GET_ITEMS() (Baldosas) (objetos myDivs)
 const elementos = RAN._get_items();
...
■■■■■■■■ GET_BOOLEAN_VALUES()
 ...
 let contenido = RAN._get_boolean_values(rango, true);	 contenido es un diccionario {id_contenido:booleano,....}
 console.log(`▶️ DICC BOOLEANO DE RANGO '${rango}' ►\n  ${JSON.stringify(contenido)}`);
■■■■■■■■ VALUES - CONTENIDO
 contenido = RAN._get_values(rango, false);			 contenido es un diccionario {id_contenido:booleano,....}
 console.log(`▶️ DICC CONTENIDO(EN BALDOSA) EN RANGO '${rango}' ►\n  ${JSON.stringify(contenido)}`);
■■■■■■■■ VALORES DE LAS CELDAS: 
 const celda = 'F0';
 contenido = RAN.valor_en_celda(celda);
 if(contenido){
...
■■■■■■■■ contenido-to-celda-R
 ...
 const celda_de_contenido = 	RAN.buscar_celda_by_contenido(id_contenido);
 console.log(`▶️ CONTENIDO '${id_contenido}' está en CELDA ► '${celda_de_contenido}'`);
■■■■■■■■ IS_R_VACIO??
 const es_vacio = RAN.es_rango_vacío(rango);
 console.log(`▶️ RANGO '${rango}' está vacio? ► ${es_vacio}`);
■■■■■■■■ BUSCAR RANGO LIBRE
 const dimension = '4x5';
 const celda_inicio = 'A0';
 const rango_free = RAN._busca_dimension_free(dimension, celda_inicio);
 if (rango_free)
...
■■■■■■■■ UNION / INTERSECCION
const celdas_union = RAN._get_union('rango_fila_0','rango_fila_1');
const celdas_intersección = RAN._get_interseccion('rango_columna_1','rango_fila_0');
const celdas_intersección_2 = RAN._get_interseccion('rango_columna_1','rango_columna_2');
const celdas_intersección_3 = RAN._get_interseccion('rango_columna_1','rango_columna_1');
console.log(`• CELDAS-UNION: ${celdas_union} ➿ ${RAN._is_continuos(celdas_union)}`);
console.log(`• CELDAS-INTERSECCION:${celdas_intersección} ➿ ${RAN._is_continuos(celdas_intersección)}`);
console.log(`• CELDAS-INTERSECCION NULL:${celdas_intersección_2} ➿ ${RAN._is_continuos(celdas_intersección_2)} `);
■■■■■■■  ES RANGO CONTINUO??
RAN._is_continuos(celdas_union);
■■■■■■■  ADD RANGO "A MANO"
RAN.registrar_ficha('rango_prueba', rango_from_reservas_bdd[0]);
■■■■■■■ Tengo un Fantasma!!!! 👻👻
// juega con to_pull para cargarlo de datos.
const nombre_ghost = RAN.crear_marco({filas:3,columnas:4});
this.d_rangos[nombre_ghost];
...
/** 
 * ■ d_rangos = {rango_matriz:{}, rango_fila_0:{}, rango_fila_1, ...}
 * ■ rango_fila_1: {celda_inicio:'A1',
 * 					celda_fin:'H1',
 * 					dimension:'1x8',
 * 					geo:{A1:{delta_x:0, delta_y:0}, B1:{delta_x:1, delta_y:0}, ...},
 * 					items:{A1:'Gran_Salon_8', B1:'Gran_Salon_9', ...}
 * 					values:{A1:'silla_2', B1:'mesa_0', C1:null, ...} }
*/
class Working_Rangos  extends Working_Celdas{
	...
	constructor(api_rangos = null) {
		...
		// ■■ Diccionario principal de rangos 
		this.d_rangos = {};							
	 ...
	 * ```javascript
	 * const dim_str = this._get_dimension_by_cicf('B2', 'D4');  ► "3x3"
	 * const dim_obj = this._get_dimension_by_cicf('B2', 'D4', false); ► { filas: 3, columnas: 3 }
	 * const dim_obj = this._get_dimension_by_cicf('B2000', 'D4', false); ► null (celda inválida)
	 * ```   */
	_get_dimension_by_cicf(celda_inicio = 'A0', celda_fin = 'A0', b_get_str = true) { 
		try {
			const inicio = this._celda_to_fc(celda_inicio);
	...
	to_pull(nombre_rango, show_all=true){
		try {
			...
			const values_rango = this._get_values(nombre_rango , show_all);					
	...
	celda_to_baldosa(celda){
		...
		const indice = this.X_to_indice_matriz(celda);
		if (indice === false) return null;

		return this._get_matriz_plana()?.[indice]?.elemento_div ?? null;
	}

	/** ## Hace una Copia de un rango registrado en d_rangos. 
	 * ### Devuelve el nuevo rango si ok y null si algo va mal. */
	copy_en_d_rangos(nombre_rango, new_nombre_rango, diccionario_to_inspect){
	...
	_get_items(nombre_rango = '', b_get_mydiv=false) {
		...
		for (let fila = fila_inicio; fila <= fila_fin; fila++) {
			for (let col = col_inicio; col <= col_fin; col++) {
				...
				const indice = this.X_to_indice_matriz(fila, col);
	...
	/**
	 * Devuelve el elemento contenido en una celda.
	 * @param {string} celda Referencia de celda, por ejemplo, "B7".
	 * @returns {HTMLElement|null} Primer elemento de la baldosa o null.
	 * @example const contenido = this.valor_en_celda('B7');
	 */
	valor_en_celda(celda = '') {
		const baldosa = this.celda_to_baldosa(celda);
		return baldosa?.firstElementChild ?? null;
	}
	 ...
	 */
	buscar_celda_by_contenido(id_contenido = '') {
	 ...
	 * @example const isEmpty = this.es_rango_vacío('rango_libre'); ► true = vacio | false = con contenido | null = rango no existe
	 */
	es_rango_vacío(nombre_rango = '') {
	...
	_busca_dimension_free(dimension = '', celda_inicio = 'A0') {
			...
			const {filas: total_filas,columnas: total_columnas,} = this._get_dimension_matriz();
	...
	__es_bloque_libre(fila_inicio = 0, columna_inicio = 0, dim_rows = 1, dim_cols = 1) {
		...
		for (let fila = fila_inicio; fila < fila_inicio + dim_rows; fila++) {
			for (let columna = columna_inicio; columna < columna_inicio + dim_cols; columna++) {
				...
				const indice = this.X_to_indice_matriz(fila, columna);
				if (indice === false) return false;
	...
	_get_geometria_relativa(foto_reservas) {
		...
		foto_reservas.forEach((reserva, i) => {
			...
			ids_items.forEach(id => {
				...
				const celda = this.buscar_celda_by_contenido(id); 
				if (celda) {
	...
	*/
	...
	__get_cicf_from_celdas(array_celdas){
		...
		const dimension = this._get_dimension_by_cicf(celda_inicio, celda_fin, false);
	...
	_crear_ficha_rango(celda_inicio = 'A0', dimension = '1x1') {
		...
		const dimension_fc = this._get_dimension_by_cicf(celda_inicio, celda_fin, false);
		...
		for (let fila = ini_fc.fila; fila <= fin_fc.fila; fila++) {
			for (let columna = ini_fc.columna; columna <= fin_fc.columna; columna++) {
				...
				const my_div = indice === false ? null : this._get_matriz_plana()[indice];
				...
				if (cuadrado?.firstElementChild) {
					ficha.values[celda] = cuadrado.firstElementChild.id;
				}
			}
		}
	 ...
	 */
	_set_rangos_fijos() {
		const { filas: total_filas, columnas: total_cols } = this._get_dimension_matriz();
		if (!Number.isInteger(total_filas) || !Number.isInteger(total_cols)) return null;
		if (total_filas <= 0 || total_cols <= 0) return null;

		const ultimo_indice = this._get_matriz_plana().length - 1;
	...
	registra_rango_matriz(ultimo_indice_matriz){
		...
		const dimension = this._get_dimension_by_cicf(celda_i, celda_f);
	...
	registra_rango_columna_s(total_filas, total_cols){
		...
		for (let c = 0; c < total_cols; c++) {
			...
			for (let f = total_filas - 1; f >= 0; f--) {
				const indice = this.X_to_indice_matriz(f, c);
				if (indice !== false) {
					fin = this.X_to_celda(indice);
					break;
				}
			}
			const dimension = this._get_dimension_by_cicf(inicio, fin);
			if (dimension && this.crear_rango(`rango_columna_${c}`, inicio, dimension)) contador++;
		}
		return contador;
	}
...
class Rango_Ghost extends Working_Rangos{
	...
	crear_marco(argumento){
		...
		if(typeof argumento === 'string'){
			...
			if(dimension_fc) {
				...
				this._crear_marco_desde_dimension(filas, columnas);
	...
	/** ## Devuelve una ficha del marco "en Salon", con valores del Dom.
	 * ### RECUERDA 
	 * #### Que los values del marco al cargar desde BDD son virtuales(no existen en el Dom).
	 * #### Al 'mover' el cursor también hay diferencias entre salon y marco.
	 * #### Y si cortas los elementos, también dejan de existir en el Dom para ser de memoria RAM.
	 * 
	 * ### ■ Lo uso en informe_marco_consola para acceder al Salon y Marco por separado.
	 * 
	 * #### Al registrar el marco en d_rangos, uso _get_values y _get_items que acceden al dom.
	 * #### De esta forma puedo dejar intacto el marco y "acceder al Salon indirectamente".
	 * #### además los 'items' y 'values' vienen normalizados como string no como objetos.
	 */
	get_ficha_marco_en_Salon(){

		// ┌■ Values e items  'en Salon'. 
        const nombre_marco = this.registrar_marco_en_d_rangos();
        const d_values_salon  = this._get_values(nombre_marco, false, false);
        const d_items_salon = this._get_items(nombre_marco, false);
        this.eliminar_rango(nombre_marco);

		const ficha_marco_salon = this._get_ficha_vacia();
		ficha_marco_salon.values = d_values_salon;
		ficha_marco_salon.items = d_items_salon;
		return ficha_marco_salon || {}
	...
	_crear_marco_desde_dimension(filas, columnas){
		...
		// ┌■  Cuando llama a crear_marco solo con dimension situa el cursor en  'A0'
		const ci = 'A0';
		...
		// ┌■ Obtengo items.
		const items = this._get_items(ghost_name, true);
		...
		// ┌■ Convierto los items y los valores en Objetos Html!
		this.d_rangos[ghost_name].items = this.#_normalizar_items_marco(items);
		this.d_rangos[ghost_name].values = this.#_normalizar_values_marco(values);
	 ...
	 * @param {String} titulo  'Mover' | 'Ghost' | 'Cortar' | 'Copiar' | 'Pegar' | 'Volver' | 'Reset' 
	 ...
	 */
    informe_marco_consola(titulo = '', separacion = 7) {
		...
		/** ### Formato centrado haciendo padding sobre el texto */
		const centrar_celda = (texto, longitud, relleno = " ") => {
			return texto
        ...
        const representar_matriz = (array_celdas, arr_values=[]) => {
			...
			filas.forEach(num => {
                ...
                columnas.forEach(letra => {
                    const celda = `${letra}${num}`;
					const idCeldaPadded = celda.padEnd(ANCHO_CELDA, ' '); // Normaliza a 4 caracteres
					const id_celda_padded_centrada = centrar_celda(celda, 5, ' ');
                    if (array_celdas.includes(celda)) {
                        if(arr_values.includes(celda))
                ...
                const fila_str = `${MARGEN}${filaVisual.join('|')}`;
		...
		const get_longitud_limpia = (str) => {
			// Esta regex busca el carácter de escape seguido de los corchetes y parámetros
			const ansiRegex = /\x1b\[[0-9;]*m/g;
			return str.replace(ansiRegex, '').length;
		};
		/** ### Retorna un id. valor puede ser string u objeto. */
		/** ### Retorna un id. valor puede ser string u objeto. */
		const normalizar_identificador = (valor) => {
			if (valor === null || valor === undefined || valor === false) return 'vacío';
			if (typeof valor === 'string' || typeof valor === 'number') return String(valor);
			return valor.id || '(elemento sin id)';
		};
		/** ### Recive 2 arrays de celdas y valida si tienen las mismas o no. */
		const tienen_celdas_iguales = (celdas_marco, celdas_salon) => {
			if (celdas_marco.length !== celdas_salon.length) return false;			
			let son_iguales = true;
			celdas_marco.forEach((celda, i) => {
				if (celda !== celdas_salon[i]) {
					son_iguales = false;
				}
			});
			return son_iguales;
		...
		/** ### Values e items en el marco */
		const values_marco = this.marco.values || {};
		const items_marco = this.marco.items || {};
		...
		// ┌• Array de string 'B4:silla_3','A4:mesa_1', . . .   del Marco
		let meta_values_marco = '';
		for (const [celda, valor] of Object.entries(values_marco)) {
			meta_values_marco += `${celda}: ${normalizar_identificador(valor)}, `;
		}
	 ...
	 * ### DESPUÉS, las acciones que se pueden hacer son cut, copy y paste sobre el Salon.
	 ...
	 * ### Pero SI actualizo las nuevas celdas(.items) con los valores teniendo en cuenta la posicion.
	 * ### • esto provoca que se puedan asignar valores y luego pueda soltarlos. 
	 * ### RECUERDA::: marco tiene unos .items que no corresponden con los values del DOM.
	 */		
	...
	*/
	copiar_marco(){
		try {
			...
			const resultado = {};
			Object.entries(baldosas).forEach(([celda, baldosa]) => {
					if (!baldosa) {
						resultado[celda] = null;
						return;
					}
	...
	*/
	...
	/**
	 * ### Convierte ids y objetos MyDiv en las baldosas DOM que usa un marco.
	 * ### Conserva las claves de celda y representa con null las baldosas inexistentes.
	 */
	#_normalizar_items_marco(items){
		if (!items || typeof items !== 'object' || Array.isArray(items)) return {};

		const baldosas = {};
		for (const [celda, item] of Object.entries(items)) {
			if (item instanceof HTMLElement) {
				baldosas[celda] = item;
			} else if (item?.elemento_div instanceof HTMLElement) {
				baldosas[celda] = item.elemento_div;
			} else if (typeof item === 'string') {
				baldosas[celda] = document.getElementById(item);
			} else {
				baldosas[celda] = null;
			}
		}
	...
	/**
	 * ### Convierte los valores de un rango en los elementos DOM que usa un marco.
	 * ### Devuelve siempre un diccionario nuevo para no modificar el rango de origen.
	 */
	#_normalizar_values_marco(values){
		if (!values || typeof values !== 'object' || Array.isArray(values)) return {};

		const elementos = {};
		for (const [celda, value] of Object.entries(values)) {
			elementos[celda] = this.#_normalizar_elemento(value);
		}
		return elementos;
	}
	...
	#_normalizar_elemento(elemento){		
		...
		return this.api_rangos.normalizar_elemento(elemento);
	}
...
class El_Rango_del_Salon extends Rango_Ghost{
	constructor(api_rangos = null){
		...
		this.reserva_range_mapper = new Reserva_Range_Mapper({
			...
			calcular_dimension: (celda_inicio, celda_fin) => this._get_dimension_by_cicf(celda_inicio, celda_fin, false),
		});
	}
